...
{
    /// <summary>
    /// Provides a startup seeding routine for OpenIddict artifacts using In-Memory stores.
    /// Seeds a default API scope and a public SPA client so developers can authenticate quickly in dev.
    /// Note: Data will be lost when application restarts since we're using In-Memory stores.
    /// </summary>
    public static class OpenIddictInMemorySeeder
    {
        /// <summary>
        ...
        {
            // Resolve OpenIddict managers from the service provider.
            // Note: With In-Memory stores, we don't need to create a scope since there's no database context.
            ...
            var spaRedirectUri = configuration["Auth:Spa:RedirectUri"] ?? "http://localhost:5173/callback";
            // Client post logout redirect URI after user logout, default is "http://localhost:5173/"
            var spaPostLogoutRedirectUri = configuration["Auth:Spa:PostLogoutRedirectUri"] ?? "http://localhost:5173/";
            ...
            if (await appManager.FindByClientIdAsync(spaClientId) is null)
            {
                ...
                // Save client to in-memory store through OpenIddict manager
